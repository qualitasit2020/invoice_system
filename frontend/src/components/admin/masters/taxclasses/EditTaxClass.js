...
class EditTaxClass extends Component {   


   constructor(props) {
    super(props);

    //this.state = {taxclass_id: '', taxclass_name: '',status: ''};
    this.state = {
      taxclass_id: '',
      taxclass_name: '',
      cgst_rate: '',
   ...
   let url = process.env.REACT_APP_BACKEND_BASE_URL +'admin/masters/taxclasses/get/'+this.props.match.params.id;
    axios.get(url)
    .then(response => { 
      
      this.setState({ 
          taxclass_id: response.data.data.taxclass_id,
          taxclass_name: response.data.data.taxclass_name,
          cgst_rate: response.data.data.cgst_rate,
          sgst_rate: response.data.data.sgst_rate,
          igst_rate: response.data.data.igst_rate,
          tds_rate: response.data.data.tds_rate,
          status: response.data.data.status
        }); 

      //console.log(this.state.taxclass_name);        
    })
   ...
   onSubmit(e) {
    ...
    const taxclasses_data = {
      taxclass_id: this.state.taxclass_id,
    ...
    let uri = process.env.REACT_APP_BACKEND_BASE_URL +'admin/masters/taxclasses/update/'+taxclasses_data.taxclass_id;    
                  ...
                  <div className="container-fluid">
                    <div className="row mb-2">
                      <div className="col-sm-6">
                        <h5 className="m-0 text-dark">Edit Tax Class</h5>
                      </div>{/* /.col */}
                      <div className="col-sm-6">
                        <ol className="breadcrumb float-sm-right">
                          <li className="breadcrumb-item"><a href="#">Home</a></li>
                          <li className="breadcrumb-item active">Edit Tax Class</li>
                        </ol>
                      </div>{/* /.col */}
                    </div>{/* /.row */}
                  </div>{/* /.container-fluid */}
            ...
            <div className="card">
              <div className="card-header">
                ...
                <span style={{float: 'right'}}>
                  <button type="button" onClick={this.onClickTaxclasses} className="btn btn-sm btn-default" ><i className="fa fa-bars mr-2" />TaxClasses</button>
                </span>
              </div>
              {/* /.card-header */}
